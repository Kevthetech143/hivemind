...
serve(async (req) => {
  ...
  try {
    ...
    const isAmbiguous = ambiguity?.[0]?.is_ambiguous === true

    let results
    let searchMode = 'standard'

    if (isAmbiguous) {
      // Multi-category search: return top result from each category
      searchMode = 'multi_category'
      const { data, error } = await supabaseClient.rpc('search_knowledge_multicategory', {
        search_query: query,
        max_categories: 5
      })
      if (error) {
    ...
    if (searchMode === 'multi_category' && results && results.length > 0) {
      const categoryResults = results.map((r: any) => ({
        category: r.category,
        query: r.query,
      ...
      return new Response(
        JSON.stringify({
          ...
          category_count: ambiguity?.[0]?.category_count || results.length,
          category_results: categoryResults,
          tip: `Narrow your search: "${query} python" or "${query} docker"`,
          query_metadata: {
            total_matches: results.length,
            search_method: 'postgres_fts_multicategory',
